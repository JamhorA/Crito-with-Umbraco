...
public class MailService : IDisposable
{
    ...
    private string _password; // Här har jag rättat stavningen
    private MailKit.Net.Smtp.SmtpClient _client;

    public MailService(string from, string smtp, int port, string username, string password) // Och här också
    {
        _from = from;
        _smtp = smtp;
        _port = port;
        _username = username;
        _password = password; // Och här också
